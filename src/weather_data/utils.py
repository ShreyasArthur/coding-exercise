...
def generate_all_years() -> list:
    """
    List all years for WeatherData objects
    """
...
def calculate_stats(years: list) -> None:
    """
    This is used when calculating stats of WeatherData objects.
    """
    ...
    usable_precip_data = WeatherData.objects.exclude(
        precipitation=MISSING_VALUE)
    station_ids = set(WeatherData.objects.values_list("station_id", flat=True))
    for year in years:
        for station_id in station_ids:
            avg_max_temp = cal_avg_max_temp(
                usable_max_data, station_id, year)
            avg_min_temp = cal_avg_min_temp(
                usable_min_data, station_id, year)
            total_precipitation = cal_total_precip(
...
def cal_avg_max_temp(usable_max_data, station_id, year):
    try:
        filtered_data = usable_max_data.filter(
...
def cal_avg_min_temp(usable_min_data, station_id, year):
    try:
        filtered_data = usable_min_data.filter(
            station_id=station_id, date__year=year)
...
def cal_total_precip(usable_precip_data, station_id, year):
    try:
